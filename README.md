...
4. **Copy the env file and fill it in:**
   ...
   | `EVENT_RETENTION_DAYS` | How many days after the event date (or creation date, if none was set) an event and its photos are auto-deleted. Optional, defaults to `30` |
   | `NEXT_PUBLIC_EVENT_RETENTION_DAYS` | Same number, duplicated with the public prefix so the app can display the actual deletion date to users. Keep in sync with `EVENT_RETENTION_DAYS` |
...
A Vercel Cron Job (`vercel.json` → `GET /api/cron/cleanup`, daily at 03:00 UTC) deletes any event whose `EVENT_RETENTION_DAYS` window (default 30) has passed since its **event date** — or since creation, if no event date was set — including its photos in Storage. Deleting the `events` row cascades to every other table, but not to the actual files sitting in the `gallery` bucket, so those are removed via the Storage API first. The event date is used rather than creation date so an event set up weeks in advance doesn't start its countdown before the round has even happened. Cron only runs once deployed to Vercel; it does nothing in local dev. Vercel's Hobby (free) plan supports one daily cron job, which is exactly what this needs.
...
- **Events auto-delete 30 days after the event date** (configurable via `EVENT_RETENTION_DAYS`; falls back to the creation date if no event date was set), added once real usage made "how do I keep the database tidy" a practical question rather than a hypothetical one. The actual deletion date is shown on the Event Hub and Gallery pages (via `src/lib/retention.ts`, shared with the cron job's own calculation so the two can't disagree) so people know to save any photos they want to keep — but there's no active reminder (email, notification) as the date approaches.
