...
- **Events auto-delete after 30 days** (configurable via `EVENT_RETENTION_DAYS`), added once real usage made "how do I keep the database tidy" a practical question rather than a hypothetical one. The actual deletion date is shown on the Event Hub and Gallery pages (computed client-side from `created_at` + `NEXT_PUBLIC_EVENT_RETENTION_DAYS`) so people know to save any photos they want to keep — but there's no active reminder (email, notification) as the date approaches.
